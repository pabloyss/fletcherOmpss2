...
// Run N steps
  ...
  for (int it=1; it<=st; it++) {
    ...
    // IN: pp,pc,qp,qc
    #pragma oss task inout (pc[0:(sx*sy*sz)-1],qc[0:(sx*sy*sz)-1],pp[0:(sx*sy*sz)-1], qp[0:(sx*sy*sz)-1])
    ...
    if (tSim >= tOut) {
      ...
      // IN: sx,sy,pc,sPtr
// #pragma oss task in (pc[0:22906303]) inout(sPtr->izStart, sPtr->iyStart, sPtr->ixStart, sPtr->fpBinary )
